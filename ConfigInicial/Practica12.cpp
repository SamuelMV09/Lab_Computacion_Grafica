...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].IncliDogZ = IncliDogZ;
...
void resetElements(void)
{
	...
	IncliDogZ = KeyFrame[0].IncliDogZ;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].IncliDogZInc = (KeyFrame[playIndex + 1].IncliDogZ - KeyFrame[playIndex].IncliDogZ) / i_max_steps; 

}

// Funciones para guardar y cargar las animaciones 

void guardarArchivo(const char* filename)
{
...
void cargarArchivo(const char* filename)
{
	...
	for (int i = 0; i < FrameIndex; i++)
	{
		file >> KeyFrame[i].dogPosX
			...
			>> KeyFrame[i].tail
			>> KeyFrame[i].IncliDog
			>> KeyFrame[i].IncliDogZ;

		// Limpiar los incrementos
		KeyFrame[i].incX = 0; KeyFrame[i].incY = 0; KeyFrame[i].incZ = 0;
		KeyFrame[i].rotDogInc = 0; KeyFrame[i].headInc = 0;
		KeyFrame[i].FLegLInc = 0; KeyFrame[i].FLegRInc = 0;
		KeyFrame[i].RLegLInc = 0; KeyFrame[i].RLegRInc = 0;
		KeyFrame[i].tailInc = 0; KeyFrame[i].IncliDogInc = 0;
		KeyFrame[i].IncliDogZInc = 0;
	}
...
int main()
{
	...
	// Intentar cargar animacion guardada
	cargarArchivo("animacion.txt");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTemp = model = glm::rotate(model, glm::radians(IncliDog), glm::vec3(1.0f, 0.0f, 0.0f)); // Inclina al perro
		modelTemp = model = glm::rotate(model, glm::radians(IncliDogZ), glm::vec3(0.0f, 0.0f, 1.0f)); // Inclina al perro de costado
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_R]) IncliDogZ += 1.0f; // Control para la caida del perro de costado
	if (keys[GLFW_KEY_F]) IncliDogZ -= 1.0f;

	if (keys[GLFW_KEY_6]) tail += 1.0f; // Control de la cola 
	if (keys[GLFW_KEY_7]) tail -= 1.0f;

	if (keys[GLFW_KEY_8]) FLegL += 1.0f; // Control de la pata izquierda 
	if (keys[GLFW_KEY_9]) FLegL -= 1.0f;
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	// Con la letra P se guarda la animacion 
	if (keys[GLFW_KEY_P])
	{
		guardarArchivo("animacion.txt");
	}

	// Con la letra M se carga la animacion del archivo
	if (keys[GLFW_KEY_M])
	{
		cargarArchivo("animacion.txt");
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			IncliDogZ += KeyFrame[playIndex].IncliDogZInc;
